public class KdTree {
    ...
    private void draw(Node n , Node parent, boolean flag) // draw all points to standard draw
    {
    	if(n == null) return;
    	StdDraw.setPenColor(StdDraw.BLACK);
    ...
    private void nearest(Node n, Point2D p, Point2D[] res) {
    	if(n==null || !n.rect.contains(p)) return;
    	
    	if(res[0] == null || (n.p.distanceSquaredTo(p) < res[0].distanceSquaredTo(p)) ){
    		res[0] = n.p;
    	}
    	nearest(n.lb,p,res);
    	nearest(n.rt,p,res);
